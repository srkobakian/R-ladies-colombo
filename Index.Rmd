...
sf_sl <- read_sf("lka_adm_slsd_20200305_shp/lka_admbnda_adm2_slsd_20200305.shp") %>%
...
```{r lagoon, fig.width = 10, dpi = 600}
ggplot(sf_sl) + 
    geom_sf() +
    geom_sf(data = sf_sl %>% filter(ADM2_EN == "[unknown]"), fill = "lightblue") + 
    facet_zoom(xy = ADM2_EN == "[unknown]", zoom.size = 0.8) +
...
sf_sl <- sf_sl %>% 
  ...
  mutate(DISTRICT = case_when(
          DISTRICT == "BATTICALOA" ~ "BATTICOLOA",
          DISTRICT == "NUWARA ELIYA" ~ "NUWARAELIYA",
          DISTRICT == "MONARAGALA" ~ "MONERAGALA",
          DISTRICT == "MULLAITIVU" ~ "MULLATIVU",
          DISTRICT == "VAVUNIYA" ~ "VAVUNIA",
          TRUE ~ DISTRICT))
```
]
...
covid_districts_sl <- covid_districts_sl %>% 
  ...
  # adjust case numbers for city of Kalmunai in Ampara 1183+227 = 1410
  mutate(Count = ifelse(DISTRICT == "AMPARA", 1410, Count)) 
```
...
```{r coviddistrictsmap, fig.width = 6, fig.height = 5, dpi = 600}
...
<img src="images/ggcont.png" style="position:absolute;bottom:0%;left:10%;height:90%;width:80%;" />

---

# Non-Contiguous Cartogram
<img src="images/ggncont.png" style="position:absolute;bottom:0%;left:10%;height:90%;width:80%;" />

---

# Dorling Cartogram
<img src="images/ggdorl.png" style="position:absolute;bottom:0%;left:10%;height:90%;width:80%;" />

---

# Hexagon Tile Map
<img src="images/gghexmap.png" style="position:absolute;bottom:0%;left:10%;height:90%;width:80%;" />
